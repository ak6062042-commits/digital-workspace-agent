...
export function WorkspaceInsightsPanel({ notifications, writingSuggestions, planner, settings, onReviewNotification, onDeleteNotification, onAnalyzeWriting, onReviewWriting, onDismissSuggestion }) {
  ...
  return (
    <>
      ...
      <div className="inspector-card">
        <div className="inspector-header"><div className="inspector-title"><FileText size={13} style={{ color: 'var(--hf-lime)' }} /> Explicit writing assist</div></div>
        <form onSubmit={analyze}><textarea value={text} onChange={(event) => setText(event.target.value)} maxLength={2000} placeholder="Paste text you want analyzed. Nothing is captured automatically." className="input-hf" style={{ width: '100%', minHeight: 72, border: '1px solid var(--hf-border-subtle)', padding: 8, marginBottom: 8 }} />
          <div style={{ display: 'flex', gap: 6 }}><select value={operation} onChange={(event) => setOperation(event.target.value)} className="input-hf" style={{ border: '1px solid var(--hf-border-subtle)', padding: 6 }}><option value="summarize">Summarize</option><option value="improve">Improve</option><option value="explain">Explain</option><option value="ideas">Ideas</option></select><button className="btn-hf-primary" disabled={busy || !text.trim()}>{busy ? 'Analyzing…' : 'Analyze locally'}</button></div>
        </form>
        {writingSuggestions.slice(0, 2).map((item) => <div key={item.id} style={{ fontSize: '11px', marginTop: 10, color: 'var(--hf-text-secondary)' }}>{item.suggestion_text}<button className="btn-hf-ghost" onClick={() => onReviewWriting(item.id)} style={{ marginLeft: 6 }}><Check size={11} /></button></div>)}
      </div>
      <div className="inspector-card">
        ...
        {(planner?.suggestions || []).slice(0, 2).map((item) => <div key={item.id} className="task-item-hf"><span style={{ fontSize: '11px', flex: 1 }}>{item.context}: {item.response}</span><button className="btn-hf-ghost" onClick={() => onDismissSuggestion(item.id)}>Dismiss</button></div>)}
